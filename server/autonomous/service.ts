...
import { loadDurableControlPlaneRecords, saveApplicationMap, saveAuditEvent, saveAutonomy, saveEngineeringMemory, saveExperiment, saveFinding, saveRepairCandidate } from "./repository";
...
export async function runDurableInspection() { const result = runInspection(); await persistLatestAudit(); return getDurableSnapshot(); }
export async function setDurableAutonomy(level: AutonomyLevel) { setAutonomyLevel(level); await saveAutonomy(level); await persistLatestAudit(); return getDurableSnapshot(); }
export async function createDurableCandidate(findingId: string, title: string) { const candidate = createCandidate(findingId, title); const finding = getSnapshot().findings.find(item => item.id === findingId); await Promise.all([persistCandidate(candidate), finding ? persistFinding(finding) : Promise.resolve(null), persistLatestAudit()]); return candidate; }
export async function reviewDurableCandidate(candidateId: string, decision: "approved" | "rejected") { const result = reviewCandidate(candidateId, decision); await Promise.all([persistCandidate(result.candidate), persistLatestAudit()]); return result; }
export async function discardDurableCandidate(candidateId: string, reason: string) { const candidate = discardCandidate(candidateId, reason); await Promise.all([persistCandidate(candidate), persistLatestAudit()]); return candidate; }
export async function requestDurablePromotion(candidateId: string) { const result = requestPromotion(candidateId); const candidate = getSnapshot().candidates.find(item => item.id === candidateId); await Promise.all([candidate ? persistCandidate(candidate) : Promise.resolve(null), persistLatestAudit()]); return result; }
export async function persistDiscoveredApplicationMap<T>(manifest: T): Promise<T> { await saveApplicationMap(`workspace-${Date.now()}`, JSON.stringify(manifest)); return manifest; }
export async function createGovernedExperiment(input: { hypothesis: string; metric: string; controlDescription: string; candidateDescription: string }) { const experimentKey = `experiment-${Date.now()}`; await saveExperiment({ experimentKey, hypothesis: input.hypothesis, metric: input.metric, controlDescription: input.controlDescription, candidateDescription: input.candidateDescription, status: "draft" }); await saveAuditEvent({ eventType: "memory", title: "Governed experiment drafted", detail: `Experiment ${experimentKey} is awaiting preview measurement and an explicit keep-or-revert decision.`, outcome: "neutral", payload: JSON.stringify({ experimentKey, ...input }) }); return { experimentKey, status: "draft" as const }; }
export async function decideGovernedExperiment(experimentKey: string, decision: "kept" | "reverted", rationale: string) { await saveExperiment({ experimentKey, hypothesis: "Recorded in existing experiment", metric: "Recorded in existing experiment", controlDescription: "Recorded in existing experiment", candidateDescription: "Recorded in existing experiment", status: decision, decisionRationale: rationale }); await saveAuditEvent({ eventType: "memory", title: `Experiment ${decision}`, detail: rationale, outcome: decision === "kept" ? "success" : "warning", payload: JSON.stringify({ experimentKey, decision }) }); return { experimentKey, status: decision, rationale }; }
